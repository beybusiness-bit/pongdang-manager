...
🔲 **9단계: 피드백톡 시스템** (기획 확정됨 — 아래 스펙 참고)

### 9단계 피드백톡 전체 기획 스펙

#### 개요
- **목적**: 근무일지 작성 후 학생별 학부모 피드백 메시지(문자/카카오톡) 초안 자동 생성
- **발송 방식**: 앱 내 직접 발송 없음. 생성된 텍스트를 **복사**하거나 **문자 발송** 기능 제공
- **대상**: 학생 1명씩 개별 생성 (또는 전체 학생 일괄 생성 후 순차 처리)

#### 워크플로우
```
근무일지 작성 완료
  ↓
학생 카드의 [피드백톡 생성] 버튼 클릭 (개별)
  또는 [전체 학생 피드백톡 생성] 버튼 (일괄)
  ↓
피드백톡 템플릿 선택 모달
  ○ 평시 피드백톡
  ○ 복귀 피드백톡
...
#### 저장 방식 (옵션 2 — 확정)
- **[저장] 버튼**: WorkLog 시트에만 저장 (근무일지는 여러 번 수정 가능)
- **[피드백톡 생성] 버튼**: 별도 버튼. 템플릿 선택 → 생성 → FeedbackTalks 시트에 저장
- 이유: 근무일지 수정과 피드백톡 생성은 독립적으로 동작해야 함
...
└─────────────────────────────────────┘
```
- 일괄: [전체 학생 피드백톡 생성] 버튼 → 각 학생마다 템플릿 선택 모달 순차 표시 → 모두 생성 후 일괄 발송 옵션

#### 템플릿 구조 (FeedbackTemplates 시트)
```
id, name (템플릿명), type (평시/복귀/시험/기타), content ({{변수}} 포함), variables (쉼표 구분 문자열)
```
...
{{특강}}
```
변수목록: `날짜,학생명,수업,숙제,test,level,score,comment,특강`

#### 변수 ↔ 근무일지 데이터 매핑
| 변수 | 근무일지 필드 | 비고 |
|---|---|---|
| `{{날짜}}` | `workLog.date` | 날짜 표기 or 주차 표기 선택 가능 |
| `{{학생명}}` | `student.name` | Students 시트 자동 |
| `{{수업}}` | `workLog.content` | |
| `{{숙제}}` | `workLog.homework` | |
| `{{test}}` | `workLog.testName` | |
| `{{level}}` | `workLog.testDifficulty` | |
| `{{score}}` | `testScore + '/' + testFullScore` | |
| `{{comment}}` | `workLog.comment` | |
| `{{특강}}` | `specialClassName + specialClassCount + '회차'` | 특강 여부에 따라 자동 생성 또는 빈 값 |

#### 빈 변수 처리 규칙
- 변수값이 비어있으면 해당 줄 전체 제거 (대시/빈칸 남기지 않음)
- 템플릿에 정의된 변수만 치환, 나머지 근무일지 데이터는 무시
- 핵심 함수:
```javascript
function generateFeedbackTalk(template, workLogData) {
  var result = template.content;
  template.variables.split(',').forEach(function(varName) {
    var value = workLogData[varName] || '';
...
```
  ⚠️ 위 함수는 기획 예시. 실제 구현 시 `<script>` 내부 백틱 금지 규칙 적용 필수.

#### 9단계에서 구현할 것 (근무일지 페이지 내 추가)
1. **피드백톡 템플릿 목록 로드** (`feedbackTemplatesData`, FeedbackTemplates 시트)
2. **[피드백톡 생성] 버튼** — 각 학생 카드 상단에 추가
3. **[전체 학생 피드백톡 생성] 버튼** — 일괄 생성용
4. **템플릿 선택 모달** (평시/복귀/시험/사용자정의)
5. **변수 매핑 + 미리보기** (수정 가능한 textarea)
6. **FeedbackTalks 시트에 저장**
7. **[문자 발송] / [복사] 버튼**
8. **주차 표기 선택 기능** (`{{날짜}}` 변수에 날짜/주차 옵션)

#### 9단계에서 구현하지 않는 것 (별도 피드백톡 관리 페이지에서 처리)
- 템플릿 생성/수정/삭제 UI
- 변수 정의 인터페이스
- 피드백톡 히스토리 조회
- 발송 이력 관리

#### 필요한 상태 변수 (9단계 시작 시 추가)
```javascript
let feedbackTemplatesData = [];   // FeedbackTemplates 시트 로드
let feedbackTalksData = [];       // FeedbackTalks 시트 로드
let currentFeedbackStudentId = null;  // 피드백톡 생성 중인 학생 ID
let selectedFeedbackTemplateId = null;  // 현재 선택한 템플릿 ID
```
🔲 **10단계: 문자 CRM 연동**
🔲 **11단계: 회의록**
🔲 **12단계: 시험대비 체크리스트**
...
### FeedbackTemplates (피드백톡 템플릿) — 9단계에서 사용
```
id, name (템플릿명), type (평시/복귀/시험/기타), content ({{변수}} 포함 템플릿 본문), variables (쉼표 구분 변수목록)
```
- `content` 안의 변수는 `{{변수명}}` 형식
- `variables`는 `"날짜,학생명,수업,숙제,test,level,score,comment,특강"` 형태
...
### 피드백톡 (FeedbackTalk) 구현 패턴 (9단계에서 사용)
- **핵심 개념**: 근무일지 데이터를 템플릿 `{{변수}}`에 치환 → 학부모 메시지 초안 생성
- **템플릿 변수 치환 순서**: 1) 값 있는 변수 치환 → 2) 빈 값 변수가 있는 줄 전체 제거 → 3) 연속 빈 줄 정리
- **`{{특강}}` 특수 처리**: `isSpecialClass === true`이면 `specialClassName + ' ' + specialClassCount + '회차'`, 아니면 빈 문자열 → 해당 줄 자동 제거
- **`{{날짜}}` 특수 처리**: 날짜 표기 or 주차 표기 선택 가능. `getWeekNumberInMonth()` 연동.
- **피드백톡 생성 버튼**: 각 학생 카드 상단 (`[삭제]` 버튼 옆). 저장과 무관하게 언제든 클릭 가능.
- **저장 독립성**: 근무일지 [저장]과 [피드백톡 생성]은 완전히 별개 동작. 저장 안 해도 피드백톡 생성 가능 (현재 입력 필드값 기준으로 생성).
- **기획 문서 원본**: 2026-04-24 세션에서 사용자가 공유한 문서 기반 (섹션 7 9단계 스펙 참고).
...
**최종 세션 날짜**: 2026-04-26. 최신 커밋: CLAUDE.md 피드백톡 스펙 추가 (`main` 브랜치).
...
2. 사용자에게 근무일지 테스트 체크리스트 보여주고 피드백 수집 (아직 테스트 안 된 상태)
3. 근무일지 피드백 반영 후 → 9단계(피드백톡) 진입
4. **9단계 진입 시**: 이 파일 섹션 7의 "9단계 피드백톡 전체 기획 스펙"을 반드시 먼저 읽고 구현 시작. 기획 내용이 이미 확정되어 있으므로 사용자에게 다시 확인하지 않아도 됨.
